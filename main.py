...
st.markdown(f"""
    ...
    /* Estilo para tablas de archivos */
    .stDataFrame, table {{
        border: 1px solid #00FF00 !important;
        color: #00FF00 !important;
    }}
...
with col_a:
    ...
    # INTENTO DE CARGA DE MÓDULO DE ARCHIVOS
    try:
        ...
        st.caption("Nota: Módulo file_manager.py no detectado en /modules")

with col_k:
    st.markdown("### 🧠 NEURAL: Kenya")
    st.caption(f"Vigilando: {app_monitor}")
    
    # INTENTO DE CARGA DE MÓDULO DE SEGURIDAD
    try:
        ...
        st.caption("Nota: Módulo security_shield.py no detectado en /modules")
